...
class Document(models.Model):
    ...
    user = models.ForeignKey(
        "users.User", on_delete=models.CASCADE, related_name="document_user",
    )
...
class Notification(models.Model):
    ...
    users = models.ManyToManyField("users.User", related_name="notification_users",)
    timestamp_created = models.DateTimeField(auto_now_add=True,)
...
class UserProfile(models.Model):
    "Generated Model"
    user = models.OneToOneField(
        "users.User", on_delete=models.CASCADE, related_name="userprofile_user",
    )
    mobile_number = models.CharField(null=True, blank=True, max_length=20,)
